//////////////////////////////////////////////////////////////////////
// $Id: FileUtils.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
#else // this breaks down if there are relative elements in the path
	std::string cpy( inPath );
    while (!cpy.empty() && cpy.back() == '/')
        cpy.pop_back();
	while (!cpy.empty() && cpy.back() != '/')
        cpy.pop_back();
    if (cpy.size() > 1 && cpy.back() == '/')
        cpy.pop_back();
	return cpy;
#endif
}
